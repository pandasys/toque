...
enum class PlayState(val compat: Int) {
...
fun buildErrorState(
  ...
  setErrorMessage(code.asCompat, Content.fetch(res))
}
...
fun PlaybackInfo.toState(): PlaybackStateCompat = buildPlaybackState {
  setState(state.compat, position.value, rate.value)
...
fun PlaybackActions.set(info: PlaybackInfo): PlaybackActions = apply {
  when (info.state) {
    PlayState.Playing -> addAll(Action.Pause, Action.Stop)
    PlayState.Paused -> addAll(Action.Play, Action.Stop)
