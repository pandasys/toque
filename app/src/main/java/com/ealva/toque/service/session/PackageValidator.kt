 ...
 *
 * Reproduce copyright notice here in case header gets accidentally removed.
 *
 * Copyright 2018 Google Inc. All rights reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
class PackageValidator(context: Context, @XmlRes xmlResId: Int) {
  ...
  /**
   * Checks whether the caller attempting to connect to the media player service, via
   * [MediaBrowserServiceCompat.onGetRoot], is known.
   *
  ...
  private fun buildCertificateAllowList(parser: XmlResourceParser): Map<String, KnownCallerInfo> {
    ...
    try {
      ...
      LOG.e(xmlException) { it("Could not read allowed callers from XML.") }
    } catch (ioException: IOException) {
      LOG.e(ioException) { it("Could not read allowed callers from XML.") }
    }
  ...
  private fun getSignatureSha256(certificate: ByteArray): String {
    ...
    try {
      ...
      LOG.e(noSuchAlgorithmException) { it("No such algorithm") }
