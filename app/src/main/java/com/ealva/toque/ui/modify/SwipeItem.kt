...
package com.ealva.toque.ui.modify
      ...
      while (true) {
        ...
        awaitPointerEventScope {
          horizontalDrag(pointerId) { change ->
            if (change.positionChange().x > 10 || offsetX.value > 0f) {
        ...
        launch {
          ...
          if (targetOffsetX.absoluteValue <= maximumWidth) {
            ...
            if (velocity >= 0f) {
              // If the velocity is low, we create a fake velocity to make it look smoother
              if (velocity <= 500f) {
                velocity = 2000f
              }
